...
def imresizevec_optimized(inimg, weights, indices, dim):
    ...
    if dim == 0:
        ...
        gathered = inimg[indices].squeeze(axis=1)
        outimg = np.sum(weights * gathered, axis=1)
    elif dim == 1:
        weights = weights[np.newaxis, :, :, np.newaxis]
        gathered = inimg[:, indices].squeeze(axis=2)
        outimg = np.sum(weights * gathered, axis=2)
    else:
        raise ValueError("dim must be 0 or 1")
    
    if inimg.dtype == np.uint8:
        np.clip(outimg, 0, 255, out=outimg)
        outimg = np.round(outimg).astype(np.uint8)
...
def imresize(I, scalar_scale=None, method='bicubic', output_shape=None, mode="vec"):
    ...
    if scalar_scale is not None:
        scalar_scale = np.float16(scalar_scale)
