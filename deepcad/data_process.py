...
class trainset(Dataset):
    def __init__(self, noise_im, coordinate_list):
        self.coordinate_list = coordinate_list
        self.noise_im = noise_im

    def __getitem__(self, index):
        noise_img = self.noise_im
        single_coordinate = self.coordinate_list[index]
        ...
        # p_exc = random.random() 
        # if p_exc < 0.5:
        #     input, input = random_transform(input, input)
        # else:
        #     input, input = random_transform(input, input)
        # print('input ---> ',input.shape)
        # input = torch.from_numpy(input.copy())   np.expand_dims(noise_patch, 0)
        input = torch.from_numpy( np.expand_dims(input.copy(), 0))
        return input
...
def test_preprocess_chooseOne(args, img_id):
    ...
    # img_list = list(os.walk(im_folder, topdown=False))[-1][-1]
    img_list = [
        f for f in list(os.walk(im_folder, topdown=False))[-1][-1]
    ...
    # img_mean = noise_im.max()
    # noise_im = noise_im/noise_im.max()*100
    # print('noise_im max -----> ',noise_im.max())
    # print('noise_im min -----> ',noise_im.min())
    if noise_im.shape[0] > args.test_datasize:
        noise_im = noise_im[0:args.test_datasize, :, :]
    # if args.print_img_name:
    print('Testing image name ::: ', im_dir)
    # print('args.test_datasize -----> ', args.test_datasize)
    print('Testing image shape ::: ', noise_im.shape)
    # Minus mean before training
    print(f'noise_im min={noise_im.min()}, max={noise_im.max()}')
    noise_im = noise_im.astype(np.float32)/args.scale_factor
    # print(f'noise_im min={noise_im.min()}, max={noise_im.max()}')
